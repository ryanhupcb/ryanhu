...
class AgentSystemTests:
    ...
    async def test_llm_integration(self):
        ...
        # 测试每个LLM
        llms = [
            ("Claude", ClaudeLLM(os.getenv("CLAUDE_API_KEY"))),
            ("Qwen", QwenLLM(os.getenv("QWEN_API_KEY"))),
            ("DeepSeek", DeepSeekLLM(os.getenv("DEEPSEEK_API_KEY")))
        ]
        
        test_message = Message(role="user", content="Hello, respond with 'OK'")
        
        for name, llm in llms:
            response = await llm.generate([test_message])
            assert "OK" in response or "ok" in response.lower(), f"{name} did not respond correctly"
    ...
    async def test_tool_execution(self):
        ...
        for tool in tools:
            # 简单测试每个工具是否可以初始化
            assert tool.name, f"Tool {tool.__class__.__name__} has no name"
            assert tool.description, f"Tool {tool.__class__.__name__} has no description"
    ...
    async def test_error_recovery(self):
        ...
        result = await recovery_system.execute_with_recovery(flaky_function)
        assert result["success"], "Error recovery failed"
        assert result["attempts"] == 3, "Incorrect retry count"
        
    async def test_performance(self):
        """测试性能"""
        analyzer = PerformanceAnalyzer()
        
        async with analyzer.measure_performance("test_operation"):
            await asyncio.sleep(0.1)
            
        # 检查是否记录了性能数据
        assert len(analyzer.performance_data) > 0, "No performance data recorded"
    ...
    async def test_cost_optimization(self):
        ...
        # 测试任务复杂度评估
        simple_task = Task(description="What is 2+2?")
        complex_task = Task(description="Create a complex distributed system with microservices architecture")
        
        assert optimizer.estimate_task_complexity(simple_task) == "simple"
        assert optimizer.estimate_task_complexity(complex_task) == "complex"

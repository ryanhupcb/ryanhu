...
class AgentStateDebugger:
    """Enhanced debug tool for tracking agent states and behaviors"""
    ...
    def record_state_change(self, old_state: str, new_state: str, reason: str):
        ...
        self.state_history.append({
            ...
            'new_state': new_state,
            'reason': reason,
            'duration': self._get_state_duration(old_state)
        })
        
        # 更新状态转换矩阵
        self.state_transition_matrix[old_state][new_state] += 1
        
        if DEBUG.TRACK_AGENT_STATES:
            logging.info(f"Agent {self.agent_id} state change: {old_state} -> {new_state} ({reason})")
    ...
    def record_task_event(self, task: Task, event_type: str):
        """记录任务事件"""
    ...
    def record_performance_metric(self, metric_name: str, value: float):
        ...
        if len(self.performance_metrics[metric_name]) > 10:
            ...
            if abs(value - mean) > 2 * std:
                logging.warning(f"Performance anomaly detected in {metric_name} for agent {self.agent_id}")
    ...
    def analyze_behavior(self) -> Dict[str, Any]:
        """分析Agent行为模式"""
        return {
            'state_transitions': dict(self.state_transition_matrix),
            'common_errors': self._analyze_errors(),
            'performance_summary': self._analyze_performance(),
            'message_patterns': self._analyze_message_patterns()
        }
    
    def _get_state_duration(self, state: str) -> Optional[float]:
        """计算状态持续时间"""
        if not self.state_history:
            return None
        last_state_change = next(
            (x for x in reversed(self.state_history) 
             if x['new_state'] == state), 
            None
        )
        if last_state_change:
            return (datetime.now() - last_state_change['timestamp']).total_seconds()
        return None
    ...
    def _analyze_errors(self) -> List[Dict[str, Any]]:
        ...
        for error in self.error_log:
            ...
            error_contexts[error_type].append(error['context'])
        
        return [{
            'type': error_type,
            'count': count,
            'contexts': list(set(error_contexts[error_type]))
        } for error_type, count in error_counts.items()]
    
    def _analyze_message_patterns(self) -> Dict[str, Any]:
        """分析消息模式"""
        ...
        return {
            'type_distribution': dict(message_types),
    ...
    def _calculate_avg_response_time(self) -> float:
        ...
        for msg in self.message_trace:
            if msg['direction'] == 'sent' and msg.get('requires_response'):
                message_pairs[msg['message_id']] = msg['timestamp']
            elif msg['direction'] == 'received' and msg['correlation_id'] in message_pairs:
                sent_time = message_pairs[msg['correlation_id']]
                response_time = (msg['timestamp'] - sent_time).total_seconds()
    ...
    def _find_relevant_agents(self, problem_type: str) -> List[str]:
        ...
        for agent_id, agent in self.framework.agents.items():
            ...
            if problem_type == "planning" and agent.role == AgentRole.PLANNER:
                ...
                relevant.append(agent_id)
            elif problem_type == "validation" and agent.role == AgentRole.VALIDATOR:
                relevant.append(agent_id)
    ...
    async def _find_prerequisites(self, state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if isinstance(state, dict):
            for key, value in state.items():
                if key != "objective":  # 避免循环
                    prerequisites.append({
                        "action": f"Achieve {key}",
...
# 继续 HyperAgentSystem 类的实现
    ...
    async def shutdown(self):
        ...
        # 保存性能分析数据
        if DEBUG.PROFILE_PERFORMANCE and hasattr(self, 'performance_analyzer'):
            await self.performance_analyzer.save_analysis()
...
class OptimizationEngine:
    ...
    async def _calculate_health_score(self, metrics: Dict[str, float]) -> float:
        """计算健康分数"""
        # 综合各项指标class HyperAgentSystem:
    """HyperAgent System Implementation"""
...
# ============================= 优化引擎补充实现 =============================
    ...
    async def _find_critical_path_length(self, workflow: Dict[str, Any]) -> float:
        ...
        for task in tasks:
            task_graph.add_node(task["id"], duration=task.get("duration", 0))
            for dep in task.get("dependencies", []):
                if dep in task_map:
        ...
        if nx.is_directed_acyclic_graph(task_graph):
            ...
            # 动态规划计算最长路径
            distances = {node: 0 for node in task_graph.nodes()}
            
            for node in topo_order:
                node_duration = task_graph.nodes[node]["duration"]
                for successor in task_graph.successors(node):
                    distances[successor] = max(
                        distances[successor],
                        distances[node] + node_duration
                    )
                distances[node] += node_duration
    ...
    async def _get_baseline_metrics(self, workflow: Dict[str, Any]) -> Dict[str, Any]:
        """获取基线指标"""
        analysis = await self.analyze_workflow(workflow)
        return analysis["metrics"]
    
    def _calculate_performance_gain(self, baseline: Dict[str, Any], 
                                   optimized: Dict[str, Any]) -> float:
        """计算性能提升"""
        baseline_duration = baseline.get("total_duration", 1)
        optimized_duration = optimized.get("total_duration", 1)
        
        if baseline_duration > 0:
            return (baseline_duration - optimized_duration) / baseline_duration
        return 0
    
    def _calculate_cost_reduction(self, baseline: Dict[str, Any], 
                                 optimized: Dict[str, Any]) -> float:
        """计算成本降低"""
        baseline_efficiency = baseline.get("resource_efficiency", 0.5)
        optimized_efficiency = optimized.get("resource_efficiency", 0.5)
        
        return optimized_efficiency - baseline_efficiency
    ...
    async def _apply_optimization(self, workflow: Dict[str, Any], 
        ...
        if opt_type == "parallelize":
            # 并行化任务
            tasks = optimized.get("tasks", [])
            for task in tasks:
                if optimization.get("target_tasks") and task["id"] in optimization["target_tasks"]:
                    task["parallel"] = True
            ...
            for task in optimized.get("tasks", []):
                for resource, amount in task.get("resources", {}).items():
                    # 优化资源使用
                    task["resources"][resource] = amount * 0.8
        
        elif opt_type == "task_merge":
            # 合并任务
            merge_targets = optimization.get("merge_targets", [])
            if len(merge_targets) >= 2:
                # 简化实现：合并第一个和第二个任务
                tasks = optimized.get("tasks", [])
                if len(tasks) >= 2:
                    tasks[0]["duration"] += tasks[1].get("duration", 0)
                    tasks.pop(1)
    ...
    async def _calculate_roi(self, optimizations: List[Dict[str, Any]]) -> float:
        """计算投资回报率"""
        total_cost = sum(opt.get("implementation_cost", 1) for opt in optimizations)
        total_benefit = sum(opt.get("expected_benefit", 0) for opt in optimizations)
        
        if total_cost > 0:
            return (total_benefit - total_cost) / total_cost
        return 0
    
    async def _is_bottleneck(self, task: Dict[str, Any], 
                           workflow: Dict[str, Any]) -> bool:
        """判断任务是否为瓶颈"""
        # 简化判断：如果任务持续时间超过平均值的2倍
        tasks = workflow.get("tasks", [])
        if not tasks:
            return False
        
        avg_duration = np.mean([t.get("duration", 0) for t in tasks])
        return task.get("duration", 0) > avg_duration * 2
    
    async def _calculate_bottleneck_severity(self, task: Dict[str, Any]) -> float:
        """计算瓶颈严重程度"""
        # 基于持续时间和依赖数量
        duration_factor = min(1.0, task.get("duration", 0) / 3600)  # 归一化到小时
        dependency_factor = min(1.0, len(task.get("dependencies", [])) / 5)
        
        return (duration_factor + dependency_factor) / 2
    
    async def _calculate_bottleneck_impact(self, task: Dict[str, Any], 
                                         workflow: Dict[str, Any]) -> float:
        """计算瓶颈影响"""
        # 计算有多少后续任务受影响
        tasks = workflow.get("tasks", [])
        task_id = task["id"]
        ...
        return affected_count / len(tasks) if tasks else 0
    ...
    async def _identify_resource_bottlenecks(self, workflow: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        resource_demand = defaultdict(list)
        
        for task in workflow.get("tasks", []):
            for resource, amount in task.get("resources", {}).items():
                resource_usage[resource] += amount * task.get("duration", 0)
        ...
        for resource, usage in resource_usage.items():
            if usage > 1000:  # 阈值
                bottlenecks.append({
                    "type": "resource",
                    "id": resource,
                    "severity": min(1.0, usage / 5000),
                    "impact": len(resource_demand[resource]) / len(workflow.get("tasks", []))
                })
    ...
    async def _optimize_task(self, bottleneck: Dict[str, Any], 
        ...
        if "efficiency" in goals:
            optimizations.append({
                "id": f"opt_parallel_{task_id}",
                "type": "parallelize",
                "description": f"Parallelize task {task_id}",
                "target_tasks": [task_id],
                "complexity": 0.3,
                "expected_benefit": bottleneck["severity"] * 0.5
            })
        
        # 资源优化建议
        if "cost" in goals:
            optimizations.append({
                "id": f"opt_resource_{task_id}",
                "type": "resource_optimization",
                "description": f"Optimize resources for task {task_id}",
                "target_tasks": [task_id],
                "complexity": 0.5,
    ...
    async def _optimize_resource(self, bottleneck: Dict[str, Any], 
        ...
        """优化资源瓶颈"""
        optimizations = []
        resource_id = bottleneck["id"]
        
        # 资源扩展建议
        optimizations.append({
            "id": f"opt_scale_{resource_id}",
            "type": "resource_scaling",
            "description": f"Scale up {resource_id}",
            "resource": resource_id,
            "scale_factor": 1.5,
            "complexity": 0.7,
            "expected_benefit": bottleneck["severity"] * 0.7
        })
        
        # 资源共享建议
        if "efficiency" in goals:
            optimizations.append({
                "id": f"opt_share_{resource_id}",
                "type": "resource_sharing",
                "description": f"Enable sharing for {resource_id}",
                "resource": resource_id,
                "complexity": 0.4,
    ...
    async def _generate_global_optimizations(self, workflow: Dict[str, Any], 
        ...
        # 任务合并优化
        if "efficiency" in goals:
            similar_tasks = await self._find_similar_tasks(workflow)
            if similar_tasks:
                optimizations.append({
                    "id": "opt_merge_tasks",
                    "type": "task_merge",
                    "description": "Merge similar tasks",
                    "merge_targets": similar_tasks[:2],
                    "complexity": 0.6,
                    "expected_benefit": 0.3
                })
        ...
        if "performance" in goals:
            optimizations.append({
                "id": "opt_reorganize",
                "type": "workflow_reorganization",
                "description": "Reorganize workflow for better performance",
                "complexity": 0.8,
    ...
    def _deduplicate_optimizations(self, optimizations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for opt in optimizations:
            opt_key = (opt["type"], opt.get("target_tasks", ()), opt.get("resource"))
        ...
        for opt in optimizations:
            ...
            for goal in goals:
                if goal in goal_weights:
                    # 基于预期收益和复杂度计算分数
                    benefit = opt.get("expected_benefit", 0)
                    complexity = opt.get("complexity", 1)
                    score += goal_weights[goal] * benefit / (complexity + 0.1)
            
            opt["priority_score"] = score
    ...
    async def _find_similar_tasks(self, workflow: Dict[str, Any]) -> List[str]:
        """查找相似任务"""
        tasks = workflow.get("tasks", [])
        ...
        # 基于资源需求分组
        for task in tasks:
            resource_sig = tuple(sorted(task.get("resources", {}).items()))
            similar_groups[resource_sig].append(task["id"])
    ...
    async def _generate_workflow_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if metrics.get("parallelism", 0) < 0.3:
            recommendations.append("Consider increasing task parallelism")
        ...
        if metrics.get("bottleneck_score", 0) > 0.7:
            recommendations.append("Address critical bottlenecks in the workflow")
...
class HyperAgentCLI:
    ...
    async def initialize(self, config_path: str = "config.yaml"):
        """初始化系统"""
        self.system = HyperAgentSystem(config_path)
        await self.system.initialize()
    
    async def run_interactive(self):
        """运行交互式CLI"""
        print("\n" + "="*60)
        print("HyperAgent System v1.0 - Interactive CLI")
        print("="*60)
        print("\nCommands:")
        print("  submit <task>  - Submit a new task")
        ...
        while self.running:
            try:
                command = input("hyperagent> ").strip()
                ...
                if cmd == "exit" or cmd == "quit":
                    ...
                    if len(parts) > 1:
                        await self.submit_task(parts[1])
                    else:
    ...
    async def submit_task(self, description: str):
        ...
        try:
            ...
            # 等待任务完成（最多30秒）
            start_time = time.time()
            while task.status not in ["completed", "failed"] and time.time() - start_time < 30:
                await asyncio.sleep(1)
                # 这里应该有一个获取任务状态的方法
            ...
            if task.status == "completed":
                ...
                print(f"\nTask is still running (status: {task.status})")
    ...
    async def show_status(self):
        ...
        try:
            ...
            print(f"\nCoordination Strategy: {framework['coordination_strategy']}")
            print(f"Total Agents: {framework['total_agents']}")
            print(f"Active Agents: {framework['active_agents']}")
            
            # 性能指标
            perf = status.get("performance", {})
            if perf:
                print(f"\nPerformance Metrics:")
                print(f"  Total Tasks: {perf.get('total_tasks', 'N/A')}")
                print(f"  Active Agents: {perf.get('active_agents', 'N/A')}")
                print(f"  Collaboration Edges: {perf.get('collaboration_edges', 'N/A')}")
            ...
            if agents:
                ...
                for agent_id, agent_info in agents.items():
                    by_role[agent_info["role"]].append(agent_info)
                
                for role, agents_list in by_role.items():
                    active = sum(1 for a in agents_list if a["status"] != "idle")
                    tasks = sum(a["current_tasks"] for a in agents_list)
                    print(f"  {role}: {len(agents_list)} agents ({active} active, {tasks} tasks)")
    ...
    async def list_agents(self):
        ...
        try:
            ...
            print("\n" + "="*60)
            print(f"{'Agent ID':<20} {'Role':<15} {'Status':<10} {'Tasks':<8} {'Confidence':<10}")
            print("="*60)
            
            for agent_id, info in sorted(agents.items()):
                print(f"{agent_id:<20} {info['role']:<15} {info['status']:<10} "
                      f"{info['current_tasks']:<8} {info['cognitive_confidence']:.2f}")
        ...
        try:
            ...
            for agent_id, info in status["agents"].items():
                if info["current_tasks"] > 0:
                    print(f"\n{info['name']} ({info['role']}):")
                    print(f"  Active tasks: {info['current_tasks']}")
                    total_tasks += info["current_tasks"]
    ...
    async def show_help(self):
        """显示帮助信息"""
        print("\n" + "="*60)
        print("HyperAgent System - Help")
        print("="*60)
        print("\nAvailable Commands:")
        print("  submit <task>  - Submit a new task to the system")
        print("                   Example: submit 'Analyze sales data for Q4'")
        print("  status         - Display current system status")
        print("  agents         - List all agents with their current state")
        print("  tasks          - Show all active tasks in the system")
        print("  help           - Display this help message")
        print("  exit/quit      - Shutdown the system and exit")
        print("\nTask Examples:")
        print("  - 'Generate a Python script for data analysis'")
        print("  - 'Create a blog post about AI trends'")
        print("  - 'Analyze the performance metrics from last week'")
        print("  - 'Plan a marketing campaign for product launch'")
        print("="*60)
    
    async def shutdown(self):
        """关闭系统"""
        print("\nShutting down HyperAgent System...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="HyperAgent System v1.0")
    ...
    parser.add_argument("--task", type=str,
                       help="Task to execute (for non-interactive mode)")
    ...
    if args.mode == "interactive":
        ...
        # 守护进程模式
        system = HyperAgentSystem(args.config)
        ...
        try:
            while True:
                ...
                logging.info(f"System status: {status['framework']['active_agents']} active agents")
        except KeyboardInterrupt:
            await system.stop()
...
async def run_tests():
    """运行测试"""
    print("Running HyperAgent System Tests...")
    
    # 创建测试系统
    system = HyperAgentSystem("test_config.yaml")
    await system.initialize()
    await system.start()
    ...
    test_tasks = [
        ...
        "Validate the security of a code snippet",
        "Coordinate resource allocation for multiple projects"
    ]
    ...
    for task_desc in test_tasks:
        print(f"\nTesting: {task_desc}")
        try:
            task = await system.submit_task(task_desc)
            
            # 等待完成
            start_time = time.time()
            while task.status not in ["completed", "failed"] and time.time() - start_time < 60:
                await asyncio.sleep(1)
            ...
            print(f"  Status: {task.status}")
            print(f"  Duration: {results[-1]['duration']:.2f}s")
    ...
    print(f"\nFinal System State:")
    print(f"  Total Agents: {final_status['framework']['total_agents']}")
    print(f"  Active Agents: {final_status['framework']['active_agents']}")
...
2. 指定配置文件：
   python hyperagent-core.py --config production.yaml

3. 守护进程模式：
   python hyperagent-core.py --mode daemon

4. 运行测试：
   python hyperagent-core.py --mode test

5. 单次任务执行：
   python hyperagent-core.py --mode daemon --task "Analyze customer data"

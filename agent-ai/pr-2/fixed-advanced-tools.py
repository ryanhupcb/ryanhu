# LocalAgentSystem - Advanced Tools & Integration (Fixed)
# 高级工具集和集成功能
...
class DatabaseTool(Tool):
    ...
    # 添加缺失的方法
    async def insert_data(self, table: str, data: Dict[str, Any], alias: str = "default") -> Dict[str, Any]:
        """插入数据"""
        # 实现插入逻辑
        return {"success": True, "message": "Data inserted"}
        
    async def update_data(self, table: str, data: Dict[str, Any], where: Dict[str, Any], alias: str = "default") -> Dict[str, Any]:
        """更新数据"""
        # 实现更新逻辑
        return {"success": True, "message": "Data updated"}
        
    async def delete_data(self, table: str, where: Dict[str, Any], alias: str = "default") -> Dict[str, Any]:
        """删除数据"""
        # 实现删除逻辑
        return {"success": True, "message": "Data deleted"}
        
    async def run_migration(self, migration_file: str, alias: str = "default") -> Dict[str, Any]:
        """运行迁移"""
        # 实现迁移逻辑
...
class APIIntegrationTool(Tool):
    ...
    async def setup_webhook(self, **kwargs) -> Dict[str, Any]:
        """设置webhook"""
        # 实现webhook设置逻辑
...
class DocumentGeneratorTool(Tool):
    ...
    async def generate_markdown(self, content: Dict[str, Any], output_path: str = None) -> Dict[str, Any]:
        """生成Markdown文档"""
        try:
            markdown_content = ""
    ...
    async def generate_html(self, content: str, output_path: str = None, template: str = None) -> Dict[str, Any]:
        """生成HTML文档"""
        try:
            if template:
                # 使用模板
                html_content = Template(template).render(content=content)
            else:
                # 基本HTML结构
                html_content = f"""
                <!DOCTYPE html>
...
class DataProcessingTool(Tool):
    ...
    async def merge_datasets(self, datasets: List[pd.DataFrame], on: str = None, how: str = "inner") -> Dict[str, Any]:
        """合并数据集"""
        try:
            if len(datasets) < 2:
                return {"success": False, "error": "Need at least 2 datasets to merge"}
    ...
    async def aggregate_data(self, data: pd.DataFrame, group_by: List[str], 
...
class NotificationTool(Tool):
    ...
    async def send_slack_message(self, webhook_url: str, message: str, 
                               channel: str = None, username: str = "Bot") -> Dict[str, Any]:
        """发送Slack消息"""
        try:

# LocalAgentSystem - Agent Implementation (Fixed)
# Agent系统实现
...
from agent_core_architecture import (
    AgentRole, TaskStatus, Task, Message, Memory, Thought,
    Tool, PythonExecutor, FileOperator, WebSearcher, LLMInterface
)
...
class ReactToTAgent(BaseAgent):
    ...
    async def tree_search(self, root: Thought, task: Task) -> Dict[str, Any]:
        ...
        while frontier and len(visited) < 50 and len(self.thought_tree) < self.max_tree_size:
            ...
            current_thought = self.thought_tree.get(thought_id)
            ...
            for child in child_thoughts:
                if len(self.thought_tree) >= self.max_tree_size:
                    break
...
{self.format_memory(recent_memory)}"""
    ...
    def format_memory(self, memory_items: List[Dict[str, Any]]) -> str:
        ...
        for item in memory_items:
            if item["type"] == "action":
                formatted.append(f"- Action: {item['tool']} with {item.get('arguments', {})}")
...
Format as a JSON list."""
                ...
                if start >= 0 and end > start:
                    ...
                    for item in task_data:
                        task = Task(
                            ...
                            metadata={"suggested_role": item.get("role", "executor")}
                        )
                        tasks.append(task)
            else:
                # 回退到简单解析
    ...
    async def execute_task(self, task: Task, agent: BaseAgent) -> Dict[str, Any]:
        """执行单个任务"""
        self.logger.info(f"Agent {agent.agent_id} executing task {task.id}")
...
class LocalAgentSystem:
    ...
    async def initialize(self):
        """初始化系统"""
        self.logger.info("Initializing Local Agent System...")
        ...
        if self.config.claude_api_key:
            from agent_core_architecture import ClaudeLLM
            claude_llm = ClaudeLLM(self.config.claude_api_key, self.config.claude_model)
            
        if self.config.deepseek_api_key:
            from agent_core_architecture import DeepSeekLLM
            deepseek_llm = DeepSeekLLM(self.config.deepseek_api_key, self.config.deepseek_model)
        else:
            # 使用本地模式
            from agent_core_architecture import DeepSeekLLM
            deepseek_llm = DeepSeekLLM(None, self.config.deepseek_model)
            
        if self.config.qwen_api_key:
            from agent_core_architecture import QwenLLM
            qwen_llm = QwenLLM(self.config.qwen_api_key, self.config.qwen_model)
            
        # 选择可用的LLM作为协调器
        orchestrator_llm = qwen_llm or claude_llm or deepseek_llm
        if not orchestrator_llm:
            raise ValueError("No LLM available for orchestrator")
            
        # 创建协调器
        self.orchestrator = MultiAgentOrchestrator(orchestrator_llm)
        
        # 创建并注册Agents
        agents = []
        
        if deepseek_llm:
            agents.append(CoderAgent("coder_1", deepseek_llm))
            
        if claude_llm:
            agents.append(ResearcherAgent("researcher_1", claude_llm))
            agents.append(ReviewerAgent("reviewer_1", claude_llm))
        elif qwen_llm:
            # 使用Qwen作为备选
            agents.append(ResearcherAgent("researcher_1", qwen_llm))
            agents.append(ReviewerAgent("reviewer_1", qwen_llm))
    ...
    async def shutdown(self):
        """关闭系统"""
        self.logger.info("Shutting down Local Agent System...")
        # 清理资源
        self.is_initialized = False
